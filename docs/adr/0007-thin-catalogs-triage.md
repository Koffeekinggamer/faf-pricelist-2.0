...
For each thin builder, **Judson only** chooses **keep** (accept thin), **replace** (Judson Drop → `replace_vendor` with a fuller file), or **ignore** (explicit add to `IGNORE_BUILDERS`). Agents never auto-ignore or auto-delete. The thin list surfaces via **Admin + CLI**; no floor Search badge in this push. Process per builder: **triage → grill with Judson**. Scan requires a live DB (Fly/Mac pull) or Dropped files — empty cloud DBs cannot triage.
